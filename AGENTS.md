...
| 2026-09-15 | ダブルブッキング対策として座席の仮押さえ（一時ロック）機能を追加。従来は決済確定の瞬間にDBで排他ロックするだけで、座席選択中の他ユーザーには空席のまま見えていた（決済の最後の最後で「すでに予約されています」と分かる状態）。t_SEAT_STOCKにf_hold_token・f_hold_expires_atを追加しf_stock_statusに3（仮押さえ中）を新設。schedules.Hold（POST /api/schedules/{id}/hold、zaseki.htmlの「次へ」押下時に呼ぶ）で対象座席を10分間ロックし、GetSeatsは仮押さえ中の座席を他ユーザーには「予約中」として返す（期限切れはバッチを使わず参照時にその場で空席扱いに変換）。schedules.ReleaseHold（POST /api/schedules/{id}/release-hold）で明示的に解放し、zaseki.html再訪問時（history.back等での「戻る」を含む全ルート）にsessionStorage.halcinema_holdを見て自動解放するよう統一。reservations.CreateReservationは決済確定時に空席（0）に加え「自分の仮押さえ（同一hold_token）」「期限切れの仮押さえ」も取得対象にし、確定時にf_hold_token/f_hold_expires_atをクリア。js/zaseki.jsの確定ボタンを非同期化し、他ユーザーに先を越された場合（409）はその座席だけ選択解除して選び直せるようにした | Claude Code |
| 2026-09-15 | ホームページ「あなたへのおすすめ」の手がかりを、お気に入りベースからAIチャットボットと同じ手がかり（予約実績・閲覧履歴・AIおすすめ履歴）に刷新。js/recommend.jsのforMe()を書き換え、予約実績（/api/me/reservations、weight=2）・閲覧履歴（localStorage.hal_viewed_movies、movie-detail.htmlが記録、weight=1）・AIチャットボットのおすすめ履歴（localStorage.hal_ai_recommended_movies、新規、weight=1）の3つでジャンルスコアを集計する方式に変更（お気に入りは入力から除外。お気に入り自体の一覧行section-favoriteは従来通り別枠で維持）。AIおすすめ履歴はjs/chatbot.jsにrecordAiRecommendedMovies()を追加し、data.recommendedMoviesを受け取るたびhal_viewed_moviesと同じ形式（新しい順・最大15件）で記録するようにして新規取得。js/home.jsのrenderRecommend()は、手がかりが一切無いときに案内文を出す実装をやめ、section-recommendごと非表示にするよう変更 | Claude Code |
| 2026-09-15 | 上記の直後、ログアウトしてもホームの「あなたへのおすすめ」が消えない不具合を修正。①js/recommend.jsのforMe()に「未ログインなら即座に手がかりを見ずnullを返す」ガードを追加（AIチャットボットのおすすめ映画自体がログイン必須なのと一致させる）。②調査の過程で本質的な原因を特定：js/chatbot.jsの会話スレッド保存キー（halcinema_chat_threads_<mode>／halcinema_chat_active_id_<mode>）が会員IDに紐づいておらず、同じブラウザで別アカウントに切り替えると前の会員のAIチャットボット履歴（おすすめ映画のやり取り含む）がそのまま見えていた。currentMemberKey()（hal_memberのidを読む、未ログインは'guest'）を追加し、キーに<mode>_<memberId>のサフィックスを付与して会員ごとに完全分離。ログイン・ログアウトは必ずページ全体の再読み込みを伴うためモジュール読み込み時に一度だけ判定すれば足りる。html/mypage.htmlのログアウト処理でhal_viewed_movies・hal_ai_recommended_moviesもクリアするよう変更（会員ごとの手がかりが別アカウントに残らないように） | Claude Code |
| 2026-09-15 | さらにその後、予約実績・閲覧履歴・AIおすすめ履歴のいずれも無いはずのユーザーでまだ「あなたへのおすすめ」が表示される件を調査。原因はバグではなく、Claude Codeの検証作業（座席仮押さえ機能・グッズ注文バックエンドのテスト）で実際にt_RESERVATION・t_GOODS_ORDERへ書き込んだテストデータがmember_id=5（goods-test@example.com）に残っており、そのアカウントでは正当に予約実績ありと判定されていたため。ユーザー指示によりホームの「あなたへのおすすめ」機能自体を削除する方針に変更：js/recommend.jsを削除、html/index.htmlの#section-recommendとjs/recommend.js読み込みを削除、js/home.jsのrenderRecommend()呼び出しを削除。js/chatbot.jsのAIおすすめ履歴記録（recordAiRecommendedMovies、hal_ai_recommended_movies）も利用者がいなくなったため削除し、html/mypage.htmlのログアウト処理からも対応するclearを削除（hal_viewed_moviesのクリアはAIチャットボット自体が引き続き使うため維持）。お気に入り一覧（section-favorite）・本日のランキング（section-ranking）は影響なし | Claude Code |
